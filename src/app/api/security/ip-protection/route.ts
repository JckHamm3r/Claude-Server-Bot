...
export async function POST(request: Request) {
  ...
  if (typeof body.ip_max_attempts === "number") {
    ...
    await setAppSetting("ip_max_attempts", String(body.ip_max_attempts));
  }
  if (typeof body.ip_window_minutes === "number") {
    ...
    await setAppSetting("ip_window_minutes", String(body.ip_window_minutes));
  }
  if (typeof body.ip_block_duration_minutes === "number") {
    ...
    await setAppSetting("ip_block_duration_minutes", String(body.ip_block_duration_minutes));
  }

  // API abuse settings
  if (typeof body.api_abuse_protection_enabled === "boolean") {
    await setAppSetting("api_abuse_protection_enabled", body.api_abuse_protection_enabled ? "true" : "false");
  }
  if (typeof body.api_abuse_max_requests === "number") {
    ...
    await setAppSetting("api_abuse_max_requests", String(body.api_abuse_max_requests));
  }
  if (typeof body.api_abuse_window_seconds === "number") {
    ...
    await setAppSetting("api_abuse_window_seconds", String(body.api_abuse_window_seconds));
  }
  if (typeof body.api_abuse_block_minutes === "number") {
    ...
    await setAppSetting("api_abuse_block_minutes", String(body.api_abuse_block_minutes));
  }

  // Fail2ban settings
  if (typeof body.fail2ban_enabled === "boolean") {
    await setAppSetting("fail2ban_enabled", body.fail2ban_enabled ? "true" : "false");
  }
  if (typeof body.fail2ban_jail === "string") {
    ...
    await setAppSetting("fail2ban_jail", jail);
  }
  if (typeof body.fail2ban_sync_interval_seconds === "number") {
    ...
    await setAppSetting("fail2ban_sync_interval_seconds", String(body.fail2ban_sync_interval_seconds));
  }
  ...
  return NextResponse.json({
    ...
    settings: {
      ...
      ip_max_attempts: parseInt(await getAppSetting("ip_max_attempts", "5"), 10),
      ip_window_minutes: parseInt(await getAppSetting("ip_window_minutes", "10"), 10),
      ip_block_duration_minutes: parseInt(await getAppSetting("ip_block_duration_minutes", "60"), 10),
    },
  });
}
