...
export async function GET(req: NextRequest) {
  // Auth is enforced by middleware for same-origin requests.
  // For cross-origin script loads (different port), middleware may not
  // catch it, so double-check auth here.
  ...
  // The script is only served to authenticated users, so embedding
  // the base path is acceptable — they already have access.
  const script = `
(function() {
