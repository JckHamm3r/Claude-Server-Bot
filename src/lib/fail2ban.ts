...
function runCmd(binary: string, args: string[], timeoutMs = 5000): { stdout: string; error?: string } {
  try {
...
export function banIPAsync(jailName: string, ip: string): void {
  ...
  execFile("fail2ban-client", ["set", jailName, "banip", ip], (err) => {
...
export function unbanIPAsync(jailName: string, ip: string): void {
  if (!isFail2BanAvailable()) return;
  execFile("fail2ban-client", ["set", jailName, "unbanip", ip], (err) => {
